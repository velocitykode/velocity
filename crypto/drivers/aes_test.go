...
func TestNewAESDriver(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "rejects undersized key for AES-128",
			key:         make([]byte, 8),
			cipher:      "AES-128-CBC",
			wantErr:     true,
		...
		{
			name:        "rejects oversized key for AES-128",
			key:         make([]byte, 32),
			cipher:      "AES-128-CBC",
			wantErr:     true,
			errContains: "invalid key length",
		...
		{
			name:        "rejects undersized key for AES-256",
			key:         make([]byte, 16),
			cipher:      "AES-256-CBC",
			wantErr:     true,
			errContains: "invalid key length",
		...
		{
			name:        "rejects oversized key for AES-256",
			key:         make([]byte, 64),
			cipher:      "AES-256-GCM",
			wantErr:     true,
			errContains: "invalid key length",
		...
		{
			name:    "accepts exact 24-byte key for AES-192-CBC",
			key:     make([]byte, 24),
			cipher:  "AES-192-CBC",
			wantErr: false,
		},
		{
			name:    "accepts exact 24-byte key for AES-192-GCM",
			key:     make([]byte, 24),
			cipher:  "AES-192-GCM",
			wantErr: false,
		},
