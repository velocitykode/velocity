...
func TestGenMail_CreatesFile(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenMail("WelcomeEmail", GenMailOptions{}); err != nil {
		t.Fatalf("GenMail() error = %v", err)
	}
...
func TestGenMail_StripsSuffix(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenMail("OrderConfirmationMail", GenMailOptions{}); err != nil {
		t.Fatalf("GenMail() error = %v", err)
	}
...
func TestGenMail_StripsMailableSuffix(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenMail("InvoiceMailable", GenMailOptions{}); err != nil {
		t.Fatalf("GenMail() error = %v", err)
	}
