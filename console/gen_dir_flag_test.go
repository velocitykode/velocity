...
// TestResolveGenDir covers the shared --dir resolver that every gen
...
// TestGen_DirFlag_WritesToCustomDir confirms each generator honours a --dir
...
func TestGen_DirFlag_WritesToCustomDir(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "gen mail", run: func(n string) error { return GenMail(n, GenMailOptions{Dir: dir}) }, input: "OrderShipped", file: "order_shipped.go", defltAt: "internal/mail"},
		{name: "gen middleware", run: func(n string) error { return GenMiddleware(n, GenMiddlewareOptions{Dir: dir}) }, input: "RateLimit", file: "rate_limit.go", defltAt: "internal/middleware"},
		{name: "gen model", run: func(n string) error { return GenModel(n, GenModelOptions{Dir: dir}) }, input: "Invoice", file: "invoice.go", defltAt: "internal/models"},
		{name: "gen notification", run: func(n string) error { return GenNotification(n, GenNotificationOptions{Dir: dir}) }, input: "PaymentFailed", file: "payment_failed.go", defltAt: "internal/notifications"},
		{name: "gen policy", run: func(n string) error { return GenPolicy(n, GenPolicyOptions{Dir: dir}) }, input: "Post", file: "post.go", defltAt: "internal/policies"},
		{name: "gen module", run: func(n string) error { return GenModule(n, GenModuleOptions{Dir: dir}) }, input: "Payment", file: "payment.go", defltAt: "internal/modules"},
		{name: "gen resource", run: func(n string) error { return GenResource(n, GenResourceOptions{Dir: dir}) }, input: "User", file: "user.go", defltAt: "internal/resources"},
		{name: "gen handler", run: func(n string) error { return GenHandler(n, GenHandlerOptions{Dir: dir}) }, input: "Dashboard", file: "dashboard.go", defltAt: "internal/handlers"},
	}
...
// TestResolveGenDir_RejectsSymlinkComponent confirms a --dir whose lexical
...
func TestResolveGenDir_RejectsSymlinkComponent(t *testing.T) {
	tmp := t.TempDir()
	outside := t.TempDir()
	t.Chdir(tmp)
	...
	if err := GenModel("Invoice", GenModelOptions{Dir: "custom/models"}); err == nil {
...
// TestGenHandler_RejectsSymlinkInNameNesting confirms the handler guard
...
func TestGenHandler_RejectsSymlinkInNameNesting(t *testing.T) {
	tmp := t.TempDir()
	outside := t.TempDir()
	t.Chdir(tmp)
	...
	err := GenHandler("Admin/Dashboard", GenHandlerOptions{Dir: "internal/web/handlers"})
	if err == nil {
...
// TestGen_DirFlag_RejectsDanglingFinalSymlink confirms the generator refuses
...
func TestGen_DirFlag_RejectsDanglingFinalSymlink(t *testing.T) {
	tmp := t.TempDir()
	outside := t.TempDir()
	t.Chdir(tmp)
	...
	if err := GenModel("Invoice", GenModelOptions{Dir: "custom/output"}); err == nil {
...
// TestGen_DirFlag_RejectsTraversal confirms a traversal supplied via --dir
...
func TestGen_DirFlag_RejectsTraversal(t *testing.T) {
	tmp := t.TempDir()
	t.Chdir(tmp)

	err := GenModel("Invoice", GenModelOptions{Dir: "../../tmp/owned"})
	if err == nil {
