...
// TestValidateGenName_Charset locks the identifier charset contract for
...
func TestValidateGenName_Charset(t *testing.T) {
	accepted := []string{
		"User",
		"SendEmail",
		"user_profile",
...
// TestValidateGenNestedName locks the contract for the slash-permitting
...
func TestValidateGenNestedName(t *testing.T) {
	accepted := []string{
		"User",
		"Admin/Users",
		"admin/v2/Users",
...
// TestGen_RejectsSourceInjection asserts every Gen* generator rejects a
// name whose characters would survive into generated Go source. It mirrors
// TestGen_RejectsTraversal: one row per command so a future generator that
// skips scaffold.ValidateName fails here.
func TestGen_RejectsSourceInjection(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			run:  func(n string) error { return GenGRPCRPC("Greeter", n, GenGRPCRPCOptions{}) },
		},
	}
...
// TestGen_RejectsSlashInIdentifierNames asserts every generator whose name
...
func TestGen_RejectsSlashInIdentifierNames(t *testing.T) {
	const evil = "List/Foo"
...
// TestGenMigration_TableFlagValidation covers the --create/--table flag
...
// character named.
func TestGenMigration_TableFlagValidation(t *testing.T) {
	...
	// migration file.
	t.Chdir(t.TempDir())
	err := GenMigration("CreateUsersTable", GenMigrationOptions{Create: `users" //`})
	if err == nil {
		t.Fatal("GenMigration accepted hostile --create value")
	}
	...
	if err := GenMigration("AddIndexToUsers", GenMigrationOptions{Table: "users"}); err != nil {
		t.Fatalf("GenMigration rejected valid --table value: %v", err)
	}
	if err := GenMigration("AddIndexToOrders", GenMigrationOptions{Table: "orders;--"}); err == nil {
		t.Fatal("GenMigration accepted hostile --table value")
	}
}
