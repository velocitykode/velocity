...
func TestGenHandler_CreatesFile(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenHandler("User", GenHandlerOptions{}); err != nil {
		t.Fatalf("GenHandler() error = %v", err)
	}
...
func TestGenHandler_NestedPath(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenHandler("Admin/Dashboard", GenHandlerOptions{}); err != nil {
		t.Fatalf("GenHandler() error = %v", err)
	}
...
func TestGenHandler_AlreadyExists(t *testing.T) {
	t.Chdir(t.TempDir())

	os.MkdirAll("internal/handlers", 0755)
	os.WriteFile("internal/handlers/user.go", []byte("existing"), 0644)

	err := GenHandler("User", GenHandlerOptions{})
	if err == nil {
		t.Error("expected error when handler already exists")
	}
}

func TestGenHandler_StripsSuffix(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenHandler("UserHandler", GenHandlerOptions{}); err != nil {
		t.Fatalf("GenHandler() error = %v", err)
	}
...
func TestGenHandler_RejectsTraversal(t *testing.T) {
	cases := []string{
		"../../tmp/owned",
		"../tmp",
		"foo/../../../etc",
	...
	for _, name := range cases {
		t.Run(name, func(t *testing.T) {
			...
			err := GenHandler(name, GenHandlerOptions{})
			if err == nil {
...
func TestGenHandler_RejectsAbsolutePath(t *testing.T) {
	t.Chdir(t.TempDir())

	abs := "/tmp/velocity-test-abs"
	if err := GenHandler(abs, GenHandlerOptions{}); err == nil {
		t.Fatalf("expected error for absolute path %q", abs)
	}
}

func TestGenHandler_RejectsHiddenSegment(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenHandler(".hidden/foo", GenHandlerOptions{}); err == nil {
		t.Fatal("expected error for hidden-prefixed segment")
	}
}

func TestGenHandler_AllowsValidNamespace(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenHandler("admin/users", GenHandlerOptions{}); err != nil {
