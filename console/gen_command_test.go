...
func TestGenCommand_CreatesFile(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenCommand("SendEmails", GenCommandOptions{}); err != nil {
		t.Fatalf("GenCommand() error = %v", err)
	}
...
func TestGenCommand_StripsSuffix(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenCommand("SendEmailsCommand", GenCommandOptions{}); err != nil {
		t.Fatalf("GenCommand() error = %v", err)
	}
...
func TestGenCommand_VerifiesContent(t *testing.T) {
	t.Chdir(t.TempDir())

	if err := GenCommand("PruneExpired", GenCommandOptions{}); err != nil {
		t.Fatalf("GenCommand() error = %v", err)
	}
