...
func TestMakeGRPCService_NorthStar(t *testing.T) {
	...
	mustNotExist(t, filepath.Join("internal", "providers", "grpc_module.go"))
...
// TestMakeGRPCService_SamePackageModuleDedupesImport verifies that wiring two
...
func TestMakeGRPCService_SamePackageModuleDedupesImport(t *testing.T) {
	t.Chdir(t.TempDir())
	writeFakeGoMod(t, "acme/app")
...
func TestMakeGRPCService_SamePackageReusesAliasOnConflict(t *testing.T) {
	...
	path := filepath.Join("internal", "providers", "grpc_module.go")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatalf("read module: %v", err)
	}
...
// TestMakeGRPCService_MismatchedDirWithModuleErrors guards the unsupported
...
func TestMakeGRPCService_MismatchedDirWithModuleErrors(t *testing.T) {
	t.Chdir(t.TempDir())
	writeFakeGoMod(t, "acme/app")
	...
	if err := MakeGRPCService("Bar", MakeGRPCServiceOptions{Dir: otherDir, NoModule: true}); err != nil {
...
// TestMakeGRPCService_NoModuleSkipsWiring asserts the default-layout path also
// honors --no-module.
func TestMakeGRPCService_NoModuleSkipsWiring(t *testing.T) {
	t.Chdir(t.TempDir())
	writeFakeGoMod(t, "acme/app")

	if err := MakeGRPCService("Foo", MakeGRPCServiceOptions{NoModule: true}); err != nil {
		t.Fatalf("MakeGRPCService: %v", err)
	}
