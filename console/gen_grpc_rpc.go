...
// GenGRPCRPCOptions selects the streaming variant for the new rpc. At most
// one of Stream / ClientStream / Bidi may be set; multiple are an error.
type GenGRPCRPCOptions struct {
	Stream       bool
	ClientStream bool
	Bidi         bool
}

func (o GenGRPCRPCOptions) kind() (grpcRPCKind, error) {
	count := 0
	kind := grpcRPCUnary
	if o.Stream {
		count++
...
// GenGRPCRPC appends a new rpc to an existing service's proto file and adds
// a matching method stub on the Go impl. The service must already exist;
// run `vel gen grpc service <Name>` first.
func GenGRPCRPC(serviceArg, rpcArg string, opts GenGRPCRPCOptions) error {
	kind, err := opts.kind()
	if err != nil {
		return err
	}
