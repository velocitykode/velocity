...
// (client_auth.go): seed and read sessions through the SAME guard store and
...
// through guard.Session(...).Save and every read goes through guard.Session on
...
//     a probe request and asking the guard for the session (the same path as
...
// given guard. It builds a session through the guard's own store (the same path
...
//
// The guard is taken as a parameter (TestClient holds none), consistent with
// ActingAs. An empty data map writes nothing: the store skips an unmodified
// session, so no cookie is captured.
func (c *TestClient) WithSession(guard *guards.SessionGuard, data map[string]any) *TestClient {
	c.t.Helper()

	if guard == nil {
		c.t.Errorf("WithSession: a non-nil *guards.SessionGuard is required")
		return c
	}

	// A session cache must be attached or the guard's per-request caching
	// no-ops; mirrors ActingAs's seed request.
	w := httptest.NewRecorder()
	req := guards.WithSessionContext(httptest.NewRequest(http.MethodGet, "/", nil))

	session := guard.Session(req)
	if session == nil {
		c.t.Errorf("WithSession: guard returned no session for the seed request")
		return c
	}
...
// the guard from the client's own cookies, has key bound to value. Values are
...
func (c *TestClient) AssertSessionHas(guard *guards.SessionGuard, key string, value any) *TestClient {
	c.t.Helper()

	if guard == nil {
		c.t.Errorf("AssertSessionHas: a non-nil *guards.SessionGuard is required")
		return c
	}

	session := c.sessionFromClient(guard)
	if session == nil {
...
func (c *TestClient) AssertSessionMissing(guard *guards.SessionGuard, key string) *TestClient {
	c.t.Helper()

	if guard == nil {
		c.t.Errorf("AssertSessionMissing: a non-nil *guards.SessionGuard is required")
		return c
	}
...
// sessionFromClient asks the guard for the session carried by the client's
...
func (c *TestClient) sessionFromClient(guard *guards.SessionGuard) auth.Session {
	if guard == nil {
		return nil
	}
	return guard.Session(c.authProbeRequest())
}
