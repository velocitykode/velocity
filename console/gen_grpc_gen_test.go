...
func TestGenGRPCGen_InvokesBufWithGenerateInProtoDir(t *testing.T) {
	t.Chdir(t.TempDir())
	...
	if err := GenGRPCGen(); err != nil {
		t.Fatalf("GenGRPCGen: %v", err)
	}
...
func TestGenGRPCGen_MissingProtoDir(t *testing.T) {
	t.Chdir(t.TempDir())
	installFakeBuf(t, 0)
...
func TestGenGRPCGen_BufNotInPATH(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.MkdirAll(filepath.Join("api", "proto"), 0755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
...
func TestGenGRPCGen_BufExitsNonZero(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.MkdirAll(filepath.Join("api", "proto"), 0755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
...
// TestGenGRPCGen_StreamsBufStderr regression-guards against silencing buf's
...
func TestGenGRPCGen_StreamsBufStderr(t *testing.T) {
	t.Chdir(t.TempDir())
	if err := os.MkdirAll(filepath.Join("api", "proto"), 0755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
