...
func TestParseMakeGRPCServiceArgs(t *testing.T) {
	tests := []struct {
		name     string
		args     []string
		wantName string
		...
		},
		{
			...
			wantName: "TemplateControl",
			want:     console.MakeGRPCServiceOptions{Package: "admin"},
		},
		{
			name:     "equals form",
			args:     []string{"Foo", "--alias=foopb", "--impl-name=foo_svc"},
			wantName: "Foo",
			want:     console.MakeGRPCServiceOptions{Alias: "foopb", ImplName: "foo_svc"},
		},
		{
			name:    "unknown flag",
