...
- **The CLI uses space-separated subcommands.** `gen *` covers code
  ...
  through. An unknown command reports the full unmatched token sequence
  (up to the dispatcher's word bound), so a subcommand typo like
  `vel migrate frsh` is named in full rather than as its first word.
...
- `normalizeID` is exported as `auth.NormalizeID` for out-of-package user
  stores.

- **The auth model is chosen in code.** Velocity authenticates one
  identity store; there is no env-selected model registry, so model
  selection carries no string-lookup failure modes (unregistered name,
  stale registration, typo'd value).
