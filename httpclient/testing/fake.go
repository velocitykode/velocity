...
func recordRequest(req *http.Request) *http.Request {
	...
	if req.Body != nil && req.Body != http.NoBody {
		// Restore the in-flight body from whatever was read, even on a read
		// error, so a partial read never leaves the in-flight request with a
		// drained or nil body.
		body, _ := io.ReadAll(io.LimitReader(req.Body, maxFakeBodyBytes)) //nolint:forbidigo // bounded by io.LimitReader
