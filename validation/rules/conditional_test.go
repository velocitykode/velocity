...
func TestRequiredWithRule(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "multi field: fails when the first listed field is present",
			field:   "city",
			value:   nil,
			params:  []string{"address", "zip", "country"},
			data:    map[string]interface{}{"address": "123 Main St"},
			wantErr: true,
		},
		{
			name:    "multi field: fails when a later listed field is present",
			field:   "city",
			value:   nil,
			params:  []string{"address", "zip", "country"},
		...
		{
			name:    "multi field: fails when a middle listed field is present",
			field:   "city",
			value:   "",
			params:  []string{"address", "zip", "country"},
			data:    map[string]interface{}{"zip": "10001"},
			wantErr: true,
		},
		{
			name:    "multi field: passes when none of the listed fields is present",
			field:   "city",
			value:   nil,
			params:  []string{"address", "zip", "country"},
			data:    map[string]interface{}{"unrelated": "x"},
			wantErr: false,
		},
		{
			name:    "multi field: passes when a listed field is present and value is present",
			field:   "city",
			value:   "New York",
			params:  []string{"address", "zip"},
			data:    map[string]interface{}{"zip": "10001"},
			wantErr: false,
		},
		{
			name:    "multi field: fails when every listed field is present",
			field:   "city",
			value:   nil,
			params:  []string{"address", "zip"},
			data:    map[string]interface{}{"address": "123 Main St", "zip": "10001"},
			wantErr: true,
		},
	}
...
func TestRequiredWithoutRule(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "multi field: fails when the first listed field is absent",
			field:   "phone",
			value:   nil,
			params:  []string{"email", "fax", "telex"},
			data:    map[string]interface{}{"fax": "1", "telex": "2"},
			wantErr: true,
		},
		{
			name:    "multi field: fails when a later listed field is absent",
			field:   "phone",
			value:   nil,
			params:  []string{"email", "fax", "telex"},
			data:    map[string]interface{}{"email": "a@b.com", "fax": "1"},
			wantErr: true,
		},
		{
			name:    "multi field: passes when every listed field is present",
			field:   "phone",
			value:   nil,
			params:  []string{"email", "fax"},
			data:    map[string]interface{}{"email": "a@b.com", "fax": "1"},
			wantErr: false,
		},
		{
			name:    "multi field: passes when a listed field is absent and value is present",
			field:   "phone",
			value:   "555-1234",
			params:  []string{"email", "fax"},
		...
		{
			name:    "multi field: fails when none of the listed fields is present",
			field:   "phone",
			value:   "",
			params:  []string{"email", "fax"},
			data:    map[string]interface{}{},
		...
		{
			name:    "multi field: passes when every listed field is present with an empty value",
			field:   "phone",
			value:   nil,
			params:  []string{"email", "fax"},
			data:    map[string]interface{}{"email": "", "fax": nil},
			wantErr: false,
		},
	}
