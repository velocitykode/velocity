...
func TestNewAESDriver(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "derives key via HKDF when too short for AES-128",
			key:     make([]byte, 8),
			cipher:  "AES-128-CBC",
			wantErr: false,
		},
		{
			name:    "derives key via HKDF when too long for AES-128",
			key:     make([]byte, 32),
			cipher:  "AES-128-CBC",
			wantErr: false,
		},
		{
			name:    "derives key via HKDF when too short for AES-256",
			key:     make([]byte, 16),
			cipher:  "AES-256-CBC",
			wantErr: false,
		},
		{
			name:    "derives key via HKDF when too long for AES-256",
			key:     make([]byte, 64),
			cipher:  "AES-256-GCM",
			wantErr: false,
		},
