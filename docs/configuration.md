...
| `SESSION_REMEMBER_LIFETIME` | `config.go` | `0` (30 days) | no | longer-lived stolen remember cookies | minutes; lifetime of the remember-me cookie, independent of the session lifetime. When the session has ended, a valid remember cookie signs the user back in on a new session (a new sign-in, so the absolute cap restarts) and is reissued for another full lifetime. `0` = 30 days; negative is rejected. Revocation (`RevokeSession`, `RevokeAllSessions`, logout) is never revived |
