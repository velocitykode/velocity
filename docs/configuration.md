...
| `SESSION_REMEMBER_LIFETIME` | `config.go` | `0` (30 days) | no | longer-lived stolen remember cookies | minutes; lifetime of the remember-me credential from its issue, independent of the session lifetime. The issue time is sealed in the encrypted cookie and checked on the server, so a copy replayed past its lifetime is refused whatever its Max-Age said. When the session has ended, a valid remember cookie signs the user back in on a new session (a new sign-in, so the absolute cap restarts) and is reissued for another full lifetime. `0` = 30 days; negative is rejected. Revocation is never revived: a request naming a deleted server record burns the remember credential it presents; `RevokeSession` and `RevokeAllSessions` end the user's remember credential and return an error matching `ErrRememberClearPartial` when they could not (a failed user lookup, clear or record read leaves it able to sign in): keep the degraded status and clear by the user, `RevokeAllSessions(userID)` retries that work (the error, a `*RememberClearError`, names the user when the record could be read); retrying `RevokeSession` cannot, since its record is already deleted; logout ends it best effort (a failed user-store call is logged); a remember-me sign-in and logout in one request leave no remember credential. Records deleted through the session store directly (not the manager) keep remember-me. With `SESSION_STORE=server` a record the cache dropped (TTL or eviction) cannot be told from a revoked one and is treated the same way |
