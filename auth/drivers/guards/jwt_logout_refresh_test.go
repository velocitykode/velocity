...
// jwtLogoutRefreshProvider returns the user for any id so the
// JWTManager.RefreshToken path can resolve through it.
type jwtLogoutRefreshProvider struct {
	user *jwtLogoutRefreshUser
}

func (p *jwtLogoutRefreshProvider) FindByID(id interface{}) (auth.Authenticatable, error) {
	return p.user, nil
}
func (p *jwtLogoutRefreshProvider) FindByCredentials(map[string]interface{}) (auth.Authenticatable, error) {
	return p.user, nil
}
func (p *jwtLogoutRefreshProvider) ValidateCredentials(auth.Authenticatable, map[string]interface{}) bool {
	return true
}
func (p *jwtLogoutRefreshProvider) UpdateRememberToken(auth.Authenticatable, string) error {
	return nil
}

// TestJWTGuard_Logout_BumpsRefreshGeneration is the H-07 regression test.
...
func TestJWTGuard_Logout_BumpsRefreshGeneration(t *testing.T) {
	...
	_, err = guard.RefreshToken(refreshToken)
	if err == nil {
...
func (p *jwtLogoutRefreshProvider) FindByIDCtx(_ context.Context, id interface{}) (auth.Authenticatable, error) {
	return p.FindByID(id)
}
func (p *jwtLogoutRefreshProvider) FindByCredentialsCtx(_ context.Context, credentials map[string]interface{}) (auth.Authenticatable, error) {
	return p.FindByCredentials(credentials)
}
func (p *jwtLogoutRefreshProvider) UpdateRememberTokenCtx(_ context.Context, user auth.Authenticatable, token string) error {
