...
// Message type and its validation/attachment machinery (which reference
// them) live here. The mail package re-exports each under its conventional
// name (e.g. mail.ErrInvalidHeader) so existing errors.Is checks keep
// matching against either identity.
var (
	...
	// AttachFile is not a free pass to read arbitrary absolute paths:
	// applications must explicitly opt in to a containment root, matching
	// the rule that filesystem accesses are sandboxed by the kernel via
	// os.Root.
	...
	// addr-spec: comma/semicolon lists, display-name forms passed in the
	// email parameter, inputs net/mail.ParseAddress cannot resolve to one
	// mailbox, and inputs carrying the address-grammar specials <>,; are
	// all rejected.
...
// MailManager is the interface satisfied by the mail manager. It covers the
// channel registration and dispatch surface used to route a *Message through
// a named mailer. Implemented by *mail.Manager.
type MailManager interface {
	SetAttachmentRoot(root *os.Root)
	SetEventDispatcher(fn func(ctx context.Context, event interface{}) error)
	Channel(name string) (Mailer, error)
	SetChannel(name string, mailer Mailer)
	HasChannel(name string) bool
	GetChannels() []string
	Send(ctx context.Context, channel string, msg *Message) error
	Broadcast(ctx context.Context, channels []string, msg *Message) error
